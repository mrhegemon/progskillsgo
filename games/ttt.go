...
func main() {
	...
	//makes gameboard
	mtx = make([] *vector.Vector, x)
	//populates gameboard
	for n, _ := range mtx {
		mtx[n] = new(vector.Vector)
		for b := 0; b < y; b++ {
			mtx[n].Push("")
		}
	}
...
func winner(name string) games.Outcome {
	...
	bool winner = false
	for ow = 0; row < x; row++
		if (mtx[rw].At(0) == name && mtx[rw].At(1) == name && mtx[rw].At(2) == name) {
			winner = true
		} if (mtx[0].At(rw) == name && mtx[1].At(rw) == name && mtx[2].At(rw) == name) {
			winner = true
	}
